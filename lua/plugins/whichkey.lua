return {
  ...
  config = function()
    ...
    wk.add {
      ...
      {
        '<leader>ss',
        function() tsc.find_files { find_command = { 'sh', '-c', "rg '' --files-with-matches" } } end,
        desc = 'Text files',
      },
      ...
      { '<leader>lr', function() tsc.lsp_references { show_line = false } end, desc = 'References' },
      { '<leader>la', lsp.code_action, desc = 'Code action' },
      { '<leader>lf', function() lsp.format { async = true } end, desc = 'Format' },
